...
def depthFirstSearch(problem):
    ...
    """
    "*** YOUR CODE HERE ***"
    # print("Start:", len(problem.getStartState()))
    # print(problem)
    # print("Is the start a goal?", problem.isGoalState(problem.getStartState()))
    # print("Start's successors:", problem.getSuccessors(problem.getStartState()))
    ...
    cur_children = problem.getSuccessors(cur_state)
    closed_set.append(cur_state)

    for child in cur_children:
        if child[0] not in closed_set:
            fringe.push((actions.copy(), child))
    ...
    while(not fringe.isEmpty()):
        ...
        actions = cur_state[0]
        cur_action = cur_state[1][1]
        cur_cost = cur_state[1][2]
        cur_state = cur_state[1][0]

        if problem.isGoalState(cur_state):
            actions.append(cur_action)
            print("Goal Reached : ", actions)
            print("Goal Length : ", len(actions))
            return actions
        
        if (cur_state not in closed_set):  
            # print("Expanding ",cur_state)
            cur_children = problem.getSuccessors(cur_state)
            # print("Successors ",cur_children)
...
def breadthFirstSearch(problem):
    ...
    while(not fringe.isEmpty()):
        ...
        # print("isGoalState : ", problem.isGoalState(cur_state))

        if problem.isGoalState(cur_state) == True:
            try:
                if problem.type == "CP" :
                    pass
...
def uniformCostSearch(problem):
    ...
    for child in cur_children:
        if child[0] not in closed_set:
            fringe.update((actions.copy(), child, total_cost), total_cost+child[2])
...
def biDirectionalMeetInTheMiddle(problem, heuristic=nullHeuristic):
    ...
    while (not forward_open_fringe.isEmpty() and not backward_open_fringe.isEmpty()):
        ...
        if U <= max(C, forward_open_fringe.prmin_node().getF(), backward_open_fringe.prmin_node().getF(), forward_g.prmin_node().getG() + backward_g.prmin_node().getG() + eps):
            # return U

            print("Nodes generated : {}".format( len(forward_map) + len(backward_map) + len(forward_closed_list) + len(backward_closed_list) + 1 ))
            print("Cost: {}".format(U))
        ...
        if C == forward_open_fringe.prmin():
            ...
            for c in successors:
                ...
                if child_node != None: # c is present in Forward U Backward maps
                    ...
                    child_node = mm_state(child_state, child_node_action_sequence, child_node_cost, child_node_heuristic, child_node_pr)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    for child in cur_children:
        if child[0] not in closed_set:
            fringe.update((actions.copy(), child, total_cost), total_cost+child[2]+heuristic(child[0],problem))
