...
def path_update(problem, fringe1, fringe2, closed1, fringe_o_nodes1, fringe_o_nodes2, cost1, cost2, paths1, itr1, itr2, direction="forward"):
    fringe1.push(itr1, 2 * cost2[itr1])
    fringe_o_nodes1.add(itr1)
    closed1.add(itr2)
    if direction == "forward":
        children = problem.getSuccessors(itr2)
    else:
        children = problem.getSuccessors(itr2)
...
def MM0Search(problem):
    ...
    fringe_openb.push(problem.getGoalState(), 2 * gb_goal)
    fringe_openb_nodes = set()
    closed_b = set()
    paths_b = {problem.getGoalState(): []}
    cost_b = {problem.getGoalState(): gb_goal}

    fringe_openf_nodes.add(problem.getStartState())
    fringe_openb_nodes.add(problem.getGoalState())
...
def bidirectional_MM(problem, heuristic=manhattanHeuristic):
    """
    ...
    def bidirectional_main():
        ...
        while ((open_forward.isEmpty() == False) and (open_backward.isEmpty() == False)):
            ...
            # check if C is equal to the minimum priority of the backward queue
                ...
                children_backward = problem.getSuccessorsBS(state_backward)
...
def bidirectional_MM0(problem, heuristic=nullHeuristic):
    """
    This function gets the minimum f value in the open_queue and g_value
    data structures. It goes through every single node in open_queue and
    then calculates the f_value of the node by adding the heuristic with
